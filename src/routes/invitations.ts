...
export async function invitationRoutes(app: FastifyInstance) {
  app.post("/api/v1/members/invite", async (request, reply) => {
    ...
    const existing = await findAuthUserByEmail(admin, body.data.email);
    if (existing.error) {
      request.log.error({ code: existing.error.code }, "Auth user lookup failed");
      return reply.code(500).send({ error: "USER_LOOKUP_FAILED" });
    }
    ...
    if (userId) {
      const { data: existingProfile, error: profileError } = await admin
        ...
        .maybeSingle();
      if (profileError) {
        request.log.error({ code: profileError.code }, "Existing profile lookup failed");
        return reply.code(500).send({ error: "USER_PROFILE_LOOKUP_FAILED" });
      }
      if (existingProfile) {
        return reply.code(409).send({ error: "USER_ALREADY_LINKED" });
      }
    } else {
      ...
      if (inviteError || !invited.user) {
        request.log.error({ code: inviteError?.code }, "User invitation failed");
    ...
    if (provisionError) {
      if (createdByThisRequest) await admin.auth.admin.deleteUser(userId);
